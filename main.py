...
def login():
    ...
    if verify_user(data["email"], data["password"]):
        ...
        token = jwt.encode(
            {"email": data["email"], "exp": datetime.now(datetime.UTC) + timedelta(hours=JWT_EXPIRE_HOURS)},
...
def refresh_token():
    ...
    try:
        ...
        new_token = jwt.encode(
            {"email": decoded["email"], "exp": datetime.now(datetime.UTC) + timedelta(hours=JWT_EXPIRE_HOURS)},
            JWT_SECRET, algorithm="HS256")
